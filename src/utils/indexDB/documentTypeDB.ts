import type { DocumentTypeRecord, DocumentTypeFormValues } from '@/types/documentType.types'
...
const toRecord = (
  ...
  is_document_sensitive: values.is_document_sensitive,
  is_required_document_approval: values.is_required_document_approval,
  predefined_fields: values.predefined_fields,
  fields: values.fields,
  fields_count: values.fields.length,
...
export const addDocumentType = (values: DocumentTypeFormValues): Promise<number> =>
  withStore('readwrite', (store) => store.add(toRecord(values)) as IDBRequest<number>)

export const updateDocumentType = (values: DocumentTypeFormValues & { id: number }): Promise<void> =>
  withStore('readwrite', (store) =>
    store.put(toRecord(values, values.id)) as IDBRequest<void>,
  )
...
export const duplicateDocumentType = async (id: number): Promise<number> => {
  ...
  return withStore('readwrite', (store) => store.add(duplicate) as IDBRequest<number>)
}
